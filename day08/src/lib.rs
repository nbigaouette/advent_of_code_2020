...
//!
//! Somewhere in the program, _either_ a `jmp` is supposed to be a `nop`, _or_ a `nop` is
...
//! program terminates, the accumulator contains the value _`8`_ (`acc +1`, `acc +1`, `acc +6`).
//!
//! Fix the program so that it terminates normally by changing exactly one `jmp` (to
//! `nop`) or `nop` (to `jmp`). _What is the value of the accumulator after the program
//! terminates?_
//!
